...
<body>
  ...
  <div style="margin:3vh auto auto 14vw">
	<div>
	  <span>中转城市：</span>
	 <el-radio-group v-model="radio1" @change="changeMidCity">
      <el-radio-button  v-for="item in citys" :label="item" :key="item"></el-radio-button>
    </el-radio-group>
	<span style="display: inline-block;margin: 0 10px auto 15%;">其他：</span>
    ...
    <el-button icon="el-icon-search"  onClick="newMidCity()" size="small">搜索</el-button>
	</div>
  </div>
  ...
  <div v-else style="padding: 10vh;font-size: 25px;margin-left: 15vw;">
    <span>很抱歉，按您的查询条件，当前未找到从{{fromCity}} 经{[{radio1}]} 中转到{{toCity}} 的列车，建议您更改查询条件。</span>
  </div>
  ...
  <script>
	var midCity = new Vue({
        ...
        methods: {
            changeResult() {
                url = "http://localhost:8080/resuList?from_city={{fromCity}}&to_city={{toCity}}&mid_city="+this.radio1+"&type="+this.radio2+"&typeFirst="+this.trainTypeFirst+"&typeSecond="+this.trainTypeSecond;
                console.log(url);
                xmlFun(url,getTime);
            },
            changeMidCity(){
    ...
    function getCityList(res){
        //请求可行中转城市集合
        midCity.citys = res;
        midCity.radio1 = res && res[0];
    }
	...
	xmlFun("http://localhost:8080/transCityList?from_city={{fromCity}}&to_city={{toCity}}",getCityList);
