...
#综合考量
def multJudge(start,mid,end,firstSql,secondSql):
    flag = 1
    resultList = []
    list = Interface.sort.multJu(SqlDir.train_table.selectByCity(start,mid,firstSql))
    listNext = Interface.sort.multJu(SqlDir.train_table.selectByCity(mid,end,secondSql))
...
def computeResult(list):
    for i in range(0,len(list)-1):
        for j in range(0,len(list)-i-1):
            price = int(tool.priceToFloat(list[j][0]['price'])) + int(tool.priceToFloat(list[j][1]['price']))
            price1 = int(tool.priceToFloat(list[j+1][0]['price'])) + int(tool.priceToFloat(list[j+1][1]['price']))
